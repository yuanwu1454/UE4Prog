...
// UE 引擎原生 C++ 基类 AInfo，是 Unreal 中非物理、纯逻辑、管理器型 Actor的标准父类
// AInfo 是一种没有物理实体的 Actor 基类，专门用于：
// 管理器类（GameMode、PlayerState、GameState 都继承它）
// 存储全局配置、世界数据、游戏逻辑
// 必须是 Actor 才能走 UE 的网络复制（Replication）
// ✅ 无模型、无碰撞、无渲染
// ✅ 纯逻辑管理
// ✅ 需要网络复制
// ✅ 放在场景里但看不见